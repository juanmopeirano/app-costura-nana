...
function SilFrontal({ hl }: { hl: string }) {
  return (
    <g fill={RELLENO} stroke={TRAZO} strokeWidth={1.2}>
      {/* Cabeza */}
      <circle cx={100} cy={35} r={18} />
      {/* Cuello */}
      <path d="M 88 50 L 88 64 L 112 64 L 112 50 Z" />
      {/* Torso (hombros, busto, cintura, cadera) */}
      ...
      {hl === 'hombro' && (
        <>
          <Dot x={88} y={64} />
          <Dot x={60} y={70} />
          <line x1={88} y1={64} x2={60} y2={70} stroke={HL} strokeWidth={3} />
        </>
      )}
...
function SilLateral({ hl }: { hl: string }) {
  return (
    <g fill={RELLENO} stroke={TRAZO} strokeWidth={1.2}>
      ...
      {/* Perfil lateral: busto adelante, cola atrás */}
...
function Hor({ y, x1, x2, flechas }: { y: number; x1: number; x2: number; flechas?: boolean }) {
  return (
    <g>
      <line x1={x1} y1={y} x2={x2} y2={y} stroke={HL} strokeWidth={3} />
      {flechas && (
        <>
...
function Ver({ x, y1, y2, flechas }: { x: number; y1: number; y2: number; flechas?: boolean }) {
  return (
    <g>
      <line x1={x} y1={y1} x2={x} y2={y2} stroke={HL} strokeWidth={3} />
      {flechas && (
        <>
